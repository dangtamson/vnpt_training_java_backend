var LoaiChiPhiView = function(){
	
	var that = this;
	this.AppTitle = 'Danh mục loại chi phí';
	
	this.initPage = function(){
		$('#AppTitle').html(that.AppTitle);
		oDmLoaiChiPhi.LoaiChiPhiID = 0;
		that.clearForm();
		that.bindGrid01();
		that.bindFormCancel();
	}
	...
	this.bindForm = function(){
		that.clearForm();
		$('#ma').val(oDmLoaiChiPhi.MA);
		$('#ten').val(oDmLoaiChiPhi.TEN);
	}
	...
	this.bindGrid01 = function(){
		oDmLoaiChiPhi.getAll();
        oTable.clear().draw();
        var aRows = [];
        var _html = '';
        
        for (var i = 0; i < oDmLoaiChiPhi.LIST.length; i++) {
            var _item = oDmLoaiChiPhi.LIST[i];
            var _hidden = '<input type="hidden" class="rowID" value="' + _item.LOAICHIPHIID + '" />';
            aRows.push([
                (i + 1) + _hidden,
                _item.MA,
                _item.TEN
	...
	$(function() {
		...
		that.initPage();
		 ...
		 $('.ACTIONS').on('click', '#btnSave', function () {
			 oDmLoaiChiPhi.MA = $('#ma').val();
			 oDmLoaiChiPhi.TEN = $('#ten').val();
			 var rs = oDmLoaiChiPhi.save();
			 if(rs.CODE == '0'){
				 that.initPage();
			 }
			 alert(rs.MESSAGE);
